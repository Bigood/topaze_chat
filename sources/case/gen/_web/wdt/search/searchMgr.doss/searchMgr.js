/* ========== Search manager ================================================ */
var searchMgr = {
	fPathRoot : "ide:tplFra",
	fPathSchBox : "ide:schBox",
	fPathContent : "ide:tplMain",
	fPathContentScroller : "ide:tplMain/des:div.mainContent_co",
	...
	declareIndex: function(pIdx){
		...
		this.fIdxUrl = "co/" + pIdx;
	},

	declarePageList: function(pPgeList){
		scCoLib.util.log("searchMgr.declarePageList : "+pPgeList);
	...
	reset: function(){
		...
		scCoLib.util.log("searchMgr.reset");
	...
	sPrvHit: function(){
		...
		if (tplMgr.fLft.fOpen) tplMgr.tglLft(false);
	...
	xUpdateUi: function(){
		...
		if (vRes && vRes.length > 0){
			...
			for (var i = 0; i < vRes.length; i++){
				...
				if (tplMgr.fHistMgr.isInHistory(vPgeBtn.fUrl)) vPgeBtn.onclick = function() {return scServices.scPreload.goTo(this.fUrl)};
				else {
